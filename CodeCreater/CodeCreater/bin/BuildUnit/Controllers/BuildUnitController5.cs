...
namespace RTSafe.HiddenTroubleTreatm.BusinessModules.HiddenTroubleTreatmModules.Controllers
{
    ...
    public class BuildUnitController : ModuleController
    {
        ...
        public ActionResult ListPage([DataSourceRequest]  DataSourceRequest dsRequest,Guid? segmentID,string? rememberTime)
        {
            var size = dsRequest.PageSize;
            var index = dsRequest.Page;
            var total = 0;
            var modelWrapper = buildUnit.GetAllPage(index, size, segmentID,rememberTime, out total);
