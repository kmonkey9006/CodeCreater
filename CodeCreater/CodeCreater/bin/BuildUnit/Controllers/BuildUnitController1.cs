...
namespace RTSafe.HiddenTroubleTreatm.BusinessModules.HiddenTroubleTreatmModules.Controllers
{
    ...
    public class BuildUnitController : ModuleController
    {
        ...
        public ActionResult ListPage([DataSourceRequest]  DataSourceRequest dsRequest,Guid? lineID, DateTime? sd, DateTime? ed,Guid? siteID, DateTime? sd, DateTime? ed,Decimal? money, DateTime? sd, DateTime? ed,Nullable<System.DateTime>? createTime, DateTime? sd, DateTime? ed,)
        {
            var size = dsRequest.PageSize;
            var index = dsRequest.Page;
            var total = 0;
            var modelWrapper = buildUnit.GetAllPage(index, size, lineID,siteID,money,createTime, out total);
