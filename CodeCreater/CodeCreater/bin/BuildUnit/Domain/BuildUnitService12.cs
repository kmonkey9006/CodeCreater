...
namespace RTSafe.HiddenTroubleTreatm.BusinessModules.HiddenTroubleTreatmModules.Domain
{
    ...
    public class BuildUnitService : IBuildUnitService
    {
        ...
        public List<BuildUnitModel> GetAllPage(int index, int size,Guid? lineID,Guid? segmentID,Guid? siteID,string? rememberTime, DateTime? sd, DateTime? ed, out int total)
        {
            string sql = @"select * from BuildUnit  where 1=1 ";
            if (lineID.HasValue)                sql += string.Format(" and LineID = '{0}'", lineID.Value);
            if (segmentID.HasValue)                sql += string.Format(" and SegmentID = '{0}'", segmentID.Value);
            if (siteID.HasValue)                sql += string.Format(" and SiteID = '{0}'", siteID.Value);
            if (rememberTime.HasValue)                sql += string.Format(" and RememberTime = '{0}'", rememberTime.Value);
